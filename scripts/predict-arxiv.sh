...
data_path="$GCS_DATA_PREFIX/paper_data_20200204_bert.jsonl"
now=$(date +"%Y%m%d-%H%M%S")

# dataflow job names must be in [-a-z0-9]
job_name=arxiv-prediction-scibert-"$model_name"-"$now"
job_name=${job_name//_/-}
...
if ! gsutil -q stat "$data_path"; then
  echo Did not find test data at "$data_path"
  exit 1
fi
